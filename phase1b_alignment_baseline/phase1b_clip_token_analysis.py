"""
MulBrandEval — Phase 1B CLIP Token Truncation Analysis (DrawBench / GenAI-Bench prompts)
============================================================================================
Companion to phase2b_clip_token_analysis.py and phase1a_clip_token_analysis.py.
Same measurement, applied to Phase 1B's prompt set.

No brand_name field exists in these prompts (unlike MBB briefs), so this
script only measures token counts / truncation -- no brand-survival check.

Run from MulBrandEval/ root:
    python phase1b_alignment_baseline\\phase1b_clip_token_analysis.py


Inputs:
  data/prompts/en/phase1b_prompts_en.csv   (caption)
  data/prompts/ar/phase1b_prompts_ar.csv   (caption_translated)

Outputs:
  results/phase1b/phase1b_token_comparison.csv
  results/phase1b/phase1b_token_analysis_summary.txt
"""
...
EN_PATH = Path("data/prompts/en/phase1b_prompts_en.csv")
AR_PATH = Path("data/prompts/ar/phase1b_prompts_ar.csv")

EN_CAPTION_COL = "caption"              # matches phase1b_metrics.py's LANGUAGES dict
AR_CAPTION_COL = "caption_translated"   # matches phase1b_metrics.py's LANGUAGES dict

OUTPUT_CSV     = Path("results/phase1b/phase1b_token_comparison.csv")
OUTPUT_SUMMARY = Path("results/phase1b/phase1b_token_analysis_summary.txt")
...
def main():
    ...
    n = len(merged)
    print(f"Matched {n} prompt_id rows between EN and AR Phase 1B files "
    ...
    lines = []
    lines.append("=" * 70)
    lines.append("Phase 1B — CLIP Token Analysis (DrawBench/GenAI-Bench, Arabic vs English)")
    ...
    lines.append("Arabic Phase 1B prompts are short enough to avoid truncation entirely,")
