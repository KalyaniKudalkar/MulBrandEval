"""
MulBrandEval — Phase 2A Ablation, Configuration C
===================================================
Configuration C — Flat pipeline (no short-circuit)

Runs the compiled flat LangGraph pipeline (phase2a_dag_pipeline.pipeline_flat.app)
on every generated Phase 2A image (150 briefs x 2 models = up to 300 images)
and writes one flattened row per image to:

    results/phase2a/configuration_c_flat_pipeline.csv

This is the direct comparator for Configuration B's english_ablation_table.csv
(same 300 images, same nodes, same weights — only the dependency structure
differs). Comparing the two files answers RQ3: does short-circuiting produce
measurably different (and better/more diagnostic) results than answering
every question regardless of upstream failures?
...
Place this script at:  phase2a_dag_pipeline/configuration_c_evaluate.py
Run from MulBrandEval/ root:  python -m phase2a_dag_pipeline.configuration_c_evaluate
"""
...
LANGUAGE    = "en"
MODELS      = ["sd15", "flux"]
SEED        = 42   # Phase 2A is single-seed by design

OUTPUT_FILE = Path("results/phase2a/configuration_c_flat_pipeline.csv")

# Same schema as english_ablation_table.csv (Configuration B) so the two
# files can be joined/compared directly on (brief_id, model).
OUTPUT_FIELDS = [
...
def run():
    ...
    log.info(f"Phase 2A evaluation (Configuration C — flat) — {len(jobs)} image(s) ready | "
    ...
    if skipped_no_image:
        log.info(f"{skipped_no_image} image(s) not yet generated.")
    log.info("=" * 60)
